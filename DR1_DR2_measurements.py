...
def plot_fit_DR1_DR2_PV():
    cost_f = LeastSquares(z, fsigma8_exp, sigma_fsigma8, DR1.model_wrapper_fsigma8)
    cost_DM = LeastSquares(
        z_DM, DM_over_DH_exp, sigma_DM_over_DH, DR2.model_wrapper_DM_over_DH
    )
    cost_Dv = LeastSquares(
        z_Dv, DV_over_rd_exp, sigma_DV_over_rd, DR2.model_wrapper_Dv_over_rd
    )
    combined_cost = cost_f + cost_DM + cost_Dv
    m = Minuit(
        combined_cost,
    ...
    axs[0].errorbar(
        z, fsigma8_exp, yerr=sigma_fsigma8, fmt="o", capsize=5, color="darkblue"
    )
    axs[0].plot(z_plot, fsigma8_plot, "r-", linewidth=2)
    axs[0].set_ylabel(r"$f_{\sigma8}$")
    axs[0].grid(True, alpha=0.3)
    ...
    param_text = (
        f'Fit: $\Omega_m={m.values["Omega_m"]:.3f}$, '
    ...
    plt.savefig(
        "/home/etudiant15/Documents/STAGE CPPM/Figures/chi_3combined_DESI_BAO+RSD+PV.pdf",
        bbox_inches="tight",
    )
    plt.show()
    ...
    print(
        f'BAO+RSD+PV & ${m.values["Omega_m"]:.3f}^{{+{upper_m:.3f}}}_{{{- inf_m:.3f}}}$ & ${m.values["W_0"]:.3f}^{{+{upper_0:.3f}}}_{{{- inf_0:.3f}}}$ & ${m.values["W_a"]:.3f}^{{+{upper_a:.3f}}}_{{{- inf_a:.3f}}}$ & ${m.values["sigma8"]:.3f}^{{+{upper_sigma:.3f}}}_{{{inf_sigma:.3f}}}$ & ${m.values["H_0xr_d"]:.3f}^{{+{upper_H:.3f}}}_{{{- inf_H:.3f}}}$ & {m.fval:.2f} & {m.ndof}'
    )
    return m, pars_fit
...
def plot_fit_DR1_DR2_noPV():
    cost_f = LeastSquares(
        z_noPV, fsigma8_exp_noPV, sigma_fsigma8_noPV, DR1.model_wrapper_fsigma8
    )
    cost_DM = LeastSquares(
        z_DM, DM_over_DH_exp, sigma_DM_over_DH, DR2.model_wrapper_DM_over_DH
    )
    ...
    plt.savefig(
        "/home/etudiant15/Documents/STAGE CPPM/Figures/chi_3combined_DESI_BAO+RSD.pdf",
        bbox_inches="tight",
    )
    plt.show()
    ...
    print(
        f'BAO+RSD & ${m.values["Omega_m"]:.3f}^{{+{upper_m:.3f}}}_{{{- inf_m:.3f}}}$ & ${m.values["W_0"]:.3f}^{{+{upper_0:.3f}}}_{{{- inf_0:.3f}}}$ & ${m.values["W_a"]:.3f}^{{+{upper_a:.3f}}}_{{{- inf_a:.3f}}}$ & ${m.values["sigma8"]:.3f}^{{+{upper_sigma:.3f}}}_{{{inf_sigma:.3f}}}$ & ${m.values["H_0xr_d"]:.3f}^{{+{upper_H:.3f}}}_{{{- inf_H:.3f}}}$ & {m.fval:.2f} & {m.ndof}'
    )
    return m, pars_fit
...
def plot_fit_DR1_DR2_PV_wCDM():
    ...
    plt.savefig(
        "/home/etudiant15/Documents/STAGE CPPM/Figures/mncontour_BAO+RSD+PV_wCDM.pdf",
        bbox_inches="tight",
    )
    ...
    # fig.text(0.5, 0.99, param_text)
    plt.savefig(
        "/home/etudiant15/Documents/STAGE CPPM/Figures/chi_3combined_DESI_BAO+RSD+PV_wCDM.pdf",
        bbox_inches="tight",
    )
    plt.show()
...
def plot_fit_DR1_DR2_noPV_wCDM():
    ...
    plt.axhline(-1, color="black", ls=":")
    plt.savefig(
        "/home/etudiant15/Documents/STAGE CPPM/Figures/mncontour_BAO+RSD_wCDM.pdf",
        bbox_inches="tight",
    )
    ...
    chi2_f = np.sum(
        (
            (fsigma8_exp_noPV - fonctions.fsigma8_th(z_noPV, m.values))
            / sigma_fsigma8_noPV
        )
        ** 2
    )
    ...
    plt.savefig(
        "/home/etudiant15/Documents/STAGE CPPM/Figures/chi_3combined_DESI_BAO+RSD_wCDM.pdf",
        bbox_inches="tight",
    )
    plt.show()
